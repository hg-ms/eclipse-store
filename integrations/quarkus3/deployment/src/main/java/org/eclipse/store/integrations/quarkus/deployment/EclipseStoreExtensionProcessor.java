...
import org.eclipse.store.integrations.quarkus.types.impl.BeanCreatorParameterNames;
import org.eclipse.store.integrations.quarkus.types.impl.RootCreator;
import org.eclipse.store.integrations.quarkus.types.impl.StorageBean;
import org.eclipse.store.integrations.quarkus.types.impl.StorageBeanCreator;
...
class EclipseStoreExtensionProcessor
{
    ...
    private static final String FEATURE = "eclipsestore-extension";
    ...
    {
        LOGGER.debug("BuildStep for Eclipse Store: Find any class that is annotated with @Storage");
        final Set<StorageClassInfo> rootClasses = this.findAnnotatedClasses(beanArchiveIndex);
    ...
    SyntheticBeanBuildItem rootBean(final StorageBeanBuildItem storageBeanBuildItem)
    {
        ...
        {
            LOGGER.debug("BuildStep for Eclipse Store: Configure a SyntheticBeanBuildItem for the @Storage bean");
    ...
    {
        LOGGER.debug("BuildStep for Eclipse Store: Configure a SyntheticBeanBuildItem for the bean keeping info about @Storage class");
    ...
    private Set<StorageClassInfo> findAnnotatedClasses(final BeanArchiveIndexBuildItem beanArchiveIndex)
    {
        ...
        for (AnnotationInstance ann : storageBeans)
        {
            ...
            result.add(this.createStorageBeanClassInfo(beanClassInfo));
