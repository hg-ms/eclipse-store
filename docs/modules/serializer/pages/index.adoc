...
It is usable as replacement for the default Java serialization to convert objects to a binary format and vice versa. This API is part of the persistence-binary module:
...
== Type handling
The serializer uses the {product-name}'s type handling system. This includes registration of new types on demand during serialization and xref:storage:legacy-type-mapping/index.adoc[legacy type mapping] when deserializing. This information is stored in the serializers type dictionary.
...
If you want to deserialize that with another serializer instance you must register all serialized classes or use a `TypedSerializer`!

=== Manually registering classes
You can manually register classes using the `SerializerFoundation` before you create the serializer.

[source, java]
----
final SerializerFoundation<?> foundation = SerializerFoundation.New()
	.registerEntityType(Customer.class);
final Serializer<byte[]> serializer = Serializer.Bytes(foundation);
----

=== Export and import of the type dictionary
The serializer creates a dictionary of all serialized types and their structure during the serialization process.
Those type information are required to deserialize data again. For serialized data that does not contain those type information it is possible to export all type information as a String and import them to another serializer instance.

[source, java, title="Exporting the type dictionary"]
----
final Serializer<byte[]> serializer = Serializer.Bytes();
byte[] data = serializer.serialize(customer)

String typeDictionaryString = serializer.exportTypeDictionay();
----

[source, java, title="Importing the type dictionary"]
----
...
If so, the type information is only included if there were new types registered during the current serialization.
